	...
	<div id="container">
		<div id="middle">
			...
			<table>
				...
				<c:if
					test="${paging.cri.pageNum == 1 and (keyword == null or keyword == '')}">
					<!-- 매니저글 for문 시작 -->
					<c:forEach items="${Notice}" var="Noticelist">
						<tr id="notice">
							<td class="bno"><a
								href="/board/board?category=${Noticelist.category}">공지</a></td>
							<td class="title"><a
				...
				<c:forEach items="${list }" var="boardlist">
					<tr id="listTr">
						<td class="bno"><a
							href="/board/board?category=${boardlist.category}">${boardlist.category}</a></td>
						<td class="title"><a
		...
		<div id="search">
			<div id="pagingArea">
				...
				<c:if test="${paging.prev}">
					<a
						href="/board/board?category=${paging.cri.category}&period=${paging.cri.period}&type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.startPage-1}&amount=${paging.cri.amount}"
						class="pageBtn">이전</a>
				</c:if>
				...
				<c:forEach begin="${paging.startPage}" end="${paging.endPage}"
					var="num">
					<a
						href="/board/board?category=${paging.cri.category}&period=${paging.cri.period}&type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${num}&amount=${paging.cri.amount}"
						id="pageNum"
				...
				<c:if test="${paging.next}">
					<a
						href="/board/board?category=${paging.cri.category}&period=${paging.cri.period}&type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.endPage+1}&amount=${paging.cri.amount}"
						class="pageBtn">다음</a>
				</c:if>
			</div>
		</div>
